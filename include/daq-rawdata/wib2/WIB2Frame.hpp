...
public:
    ...
    {
        if(i<0 || i>255) throw std::out_of_range("ADC index out of range");
        if(val>=(1<<14)) throw std::out_of_range("ADC value out of range");
     ...
     */
    uint16_t get_u(int femb, int i)
    {
        if(femb<0 || femb>1) throw std::out_of_range("FEMB index out of range");
        if(i<0 || i>=40) throw std::out_of_range("U channel index out of range");
     ...
     */
    uint16_t get_v(int femb, int i)
    {
        if(femb<0 || femb>1) throw std::out_of_range("FEMB index out of range");
        if(i<0 || i>=40) throw std::out_of_range("V channel index out of range");
     ...
     */
    uint16_t get_x(int femb, int i)
    {
        if(femb<0 || femb>1) throw std::out_of_range("FEMB index out of range");
        if(i<0 || i>=48) throw std::out_of_range("X channel index out of range");
    ...
    uint64_t timestamp() const {
        return (uint64_t)header.timestamp_1 | ((uint64_t)header.timestamp_2 << 32);
    }
